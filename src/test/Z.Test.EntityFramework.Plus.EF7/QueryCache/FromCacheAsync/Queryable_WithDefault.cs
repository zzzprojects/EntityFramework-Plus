...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void FromCacheAsync_Queryable_WithDefault()
        {
            ...
            {
                // BEFORE
                var itemCountBefore = ctx.Entity_Basics.FromCacheAsync().Result.Count();
                var cacheCountBefore = QueryCacheHelper.GetCacheCount();

                TestContext.DeleteAll(x => x.Entity_Basics);

                // AFTER
                var itemCountAfter = ctx.Entity_Basics.FromCacheAsync().Result.Count();
