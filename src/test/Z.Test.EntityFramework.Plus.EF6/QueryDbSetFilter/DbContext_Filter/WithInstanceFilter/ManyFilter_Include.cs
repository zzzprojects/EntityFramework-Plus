    ...
    public partial class QueryFilter_DbContext_DbSetFilter
    {
        ...
        public void WithInstanceFilter_ManyFilter_Include()
        {
            ...
            using (var ctx = new TestContext())
            {
                ...
                ctx.DbSetFilter(QueryFilterHelper.Filter.Filter2).Disable();
                ctx.DbSetFilter(QueryFilterHelper.Filter.Filter3).Disable();
                ctx.DbSetFilter(QueryFilterHelper.Filter.Filter4).Disable();

                ctx.DbSetFilter(QueryFilterHelper.Filter.Filter2).Enable(typeof (Inheritance_Interface_IEntity));
                ctx.DbSetFilter(QueryFilterHelper.Filter.Filter3).Enable(typeof (Inheritance_Interface_Base));
                ctx.DbSetFilter(QueryFilterHelper.Filter.Filter4).Enable(typeof (Inheritance_Interface_IBase));
