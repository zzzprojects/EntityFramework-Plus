...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Single_Single_Null_Single()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(single1: true, single2: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Single_Right.Single_RightRight.Single_RightRightRight)
                    .ToList().First();
