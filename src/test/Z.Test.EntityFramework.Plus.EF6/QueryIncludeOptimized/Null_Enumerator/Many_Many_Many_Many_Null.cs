...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Many_Many_Many_Many_Null()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(many1: true, many2: true, many3: true, many4: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Many_Right.SelectMany(y => y.Many_RightRight.SelectMany(z => z.Many_RightRightRight.Select(a => a.Many_RightRightRightRight))))
                    .ToList().First();
