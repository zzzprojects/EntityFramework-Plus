...
namespace Z.Test.EntityFramework.Plus
{
    public partial class QueryIncludeOptimized_Null_Enumerator
    {
        ...
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(many1: true, many2: true, single3: true, single4: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Many_Right.SelectMany(y => y.Many_RightRight.Select(z => z.Single_RightRightRight.Single_RightRightRightRight)))
                    .ToList().First();
