...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Single_Many_Many_Single_Null()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(single1: true, many2: true, many3: true, single4: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Single_Right.Many_RightRight.SelectMany(y => y.Many_RightRightRight.Select(z => z.Single_RightRightRightRight)))
                    .ToList().First();
                ...
                // Level 4
                {
                    foreach (var item in list3)
                    {
                        ...
                        Assert.IsNull(item.Many_RightRightRightRight.Count);
                    }
                }
            }
        }
