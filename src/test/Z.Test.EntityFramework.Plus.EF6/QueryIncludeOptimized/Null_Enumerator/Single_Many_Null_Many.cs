...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Single_Many_Null_Many()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(single1: true, many2: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Single_Right.Many_RightRight.SelectMany(y => y.Many_RightRightRight))
                    .ToList().First();
