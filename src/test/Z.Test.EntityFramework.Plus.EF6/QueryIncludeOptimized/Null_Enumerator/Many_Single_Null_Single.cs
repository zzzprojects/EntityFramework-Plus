...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Many_Single_Null_Single()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(many1: true, single2: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Many_Right.Select(y => y.Single_RightRight.Single_RightRightRight))
                    .ToList().First();
