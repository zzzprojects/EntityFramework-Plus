...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Many_Many_Null_Many()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(many1: true, many2: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Many_Right.SelectMany(y => y.Many_RightRight.SelectMany(z => z.Many_RightRightRight)))
                    .ToList().First();
                ...
                // Level 2
                {
                    foreach (var item in list1)
                    {
                        ...
                        Assert.AreEqual(0, item.Many_RightRight.Count);
                    }
                }
            }
        }
