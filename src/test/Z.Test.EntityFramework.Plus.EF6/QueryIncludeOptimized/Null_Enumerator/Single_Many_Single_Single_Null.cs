...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Single_Many_Single_Single_Null()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(single1: true, many2: true, single3: true, single4: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Single_Right.Many_RightRight.Select(y => y.Single_RightRightRight.Single_RightRightRightRight))
                    .ToList().First();
