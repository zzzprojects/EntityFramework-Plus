...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Many_Property_InternalAccess()
        {
            ...
            using (var ctx = new TestContext())
            {
                ...
                // AFTER
                Assert.AreEqual(3430, ctx.Internal_Entity_Basic_Manies.Sum(x => x.Column1));
                Assert.AreEqual(2440, ctx.Internal_Entity_Basic_Manies.Sum(x => x.Column2));
                Assert.AreEqual(1450, ctx.Internal_Entity_Basic_Manies.Sum(x => x.Column3));
