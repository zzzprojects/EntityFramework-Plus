...
namespace Z.Test.EntityFramework.Plus
{
    public partial class QueryCache_FromCache
    {
        ...
        public void FromCache_QueryDelayed_WithExpirations()
        {
            ...
            {
                // BEFORE
                var itemCountBefore = ctx.EntitySimples.DelayedCount().FromCache(DateTime.Now.AddMinutes(2));
                var cacheCountBefore = QueryCacheManagerHelper.GetCacheCount();

                EntitySimpleHelper.Clear();

                // AFTER
                var itemCountAfter = ctx.EntitySimples.DelayedCount().FromCache(DateTime.Now.AddMinutes(2));
