...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Many_Single_Many_Many_Null()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(many1: true, single2: true, many3: true, many4: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Many_Right.Select(y => y.Single_RightRight.Many_RightRightRight.Select(z => z.Many_RightRightRightRight)))
                    .ToList().First();
