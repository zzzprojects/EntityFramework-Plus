...
namespace Z.Test.EntityFramework.Plus
{
    ...
    {
        [TestMethod]
        public void Single_Single_Single_Single_Null()
        {
            QueryIncludeOptimizedHelper.InsertOneToOneAndMany(single1: true, single2: true, single3: true, single4: false);

            using (var ctx = new TestContext())
            {
                var left = ctx.Association_OneToSingleAndMany_Lefts
                    .IncludeOptimized(x => x.Single_Right.Single_RightRight.Single_RightRightRight.Single_RightRightRightRight)
                    .ToList().First();
