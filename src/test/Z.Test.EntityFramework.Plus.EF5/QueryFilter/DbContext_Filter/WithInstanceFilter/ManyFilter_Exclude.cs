    ...
    public partial class QueryFilter_DbContext_Filter
    {
        ...
        public void WithInstanceFilter_ManyFilter_Exclude()
        {
            using (var ctx = new EntityContext())
            {
                ...
                ctx.Filter(FilterEntityHelper.Filter.Filter2).Disable(typeof (IFilterEntity));
                ctx.Filter(FilterEntityHelper.Filter.Filter3).Disable(typeof (BaseFilterEntity));
                ctx.Filter(FilterEntityHelper.Filter.Filter4).Disable(typeof (IBaseFilterEntity));
