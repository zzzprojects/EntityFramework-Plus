	...
	public class ModelAndContext
	{
		public class My
		{
			...
			public static string ConnectionString =
				("Server=[REPLACE];Initial Catalog = [BD]; Integrated Security = true; Connection Timeout = 300; Persist Security Info=True").Replace("[REPLACE]", Environment.MachineName).Replace("[BD]", "TestEF_PLUS_NET6");
		...
		public static bool actifInterceptor = false;
		...
		class EFCommandInterceptor : IDbCommandInterceptor
		{
			public void NonQueryExecuted(System.Data.Common.DbCommand command, DbCommandInterceptionContext<int> interceptionContext)
			{
				if (actifInterceptor)
				{
					command.CommandText += "--" + String.Format(" IsAsyncé: {0}", interceptionContext.IsAsync);
				}
