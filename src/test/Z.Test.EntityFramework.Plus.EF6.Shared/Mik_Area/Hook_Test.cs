	...
	public class Hook_Test
	{
        ...
        public void Hook_Test_02()
		{
            using (var dbContext = new ModelAndContext.EntityContext())
			{
				...
				// 6 si roule avec la migration alors dans se cas le nolock ne sait pas, car bloquer par la logique actuelle qui dit si plusieurs context nope. Et sinon se fait doit juste compter correctement.
				try
				{ 
				...
				{
					Assert.AreEqual(5, sql.Split(new String[] { "NOLOCK" }, StringSplitOptions.None).LongCount());
				}
