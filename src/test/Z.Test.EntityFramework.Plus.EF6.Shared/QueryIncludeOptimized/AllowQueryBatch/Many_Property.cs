...
{
    public partial class QueryIncludeOptimized_AllowQueryBatch
    {
        [TestMethod]
        public void Many_Property()
        {
            ...
            using (var ctx = new TestContext())
            {
                var list = ctx.Association_Multi_OneToMany_Right1s
                    .Where(x => x.ColumnInt > 2)
                    .IncludeOptimized(right => right.Left)
                    .IncludeOptimized(right => right.Left.Right2s.Where(y => y.ColumnInt > 2))
                    .ToList()
                    .Select(x => x.Left)
                    .Distinct()
                    .ToList();
