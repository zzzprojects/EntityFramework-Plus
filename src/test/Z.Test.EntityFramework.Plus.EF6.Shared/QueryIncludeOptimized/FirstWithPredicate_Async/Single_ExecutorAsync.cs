...
{
    public partial class QueryIncludeOptimized_FirstWithPredicate_Async
    {
        [TestMethod]
        public async Task Single_ExecutorAsync()
        {
            ...
            using (var ctx = new TestContext())
            {
                var item = await ctx.Association_OneToMany_Lefts
                    .IncludeOptimized(left => left.Rights.Where(x => x.ColumnInt > 2))
