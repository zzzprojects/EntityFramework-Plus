...
namespace Z.EntityFramework.Plus
{
    ...
    public class QueryIncludeFilterProvider<T> : IQueryProvider
    {
        ...
        public TResult Execute<TResult>(Expression expression)
        {
            ...
            {
                var propertyY = result.GetType().GetProperty("q");
                var valueY = propertyY.GetValue(result, null);
                var t2 = valueY.GetType().GetGenericArguments()[0];
                var toListMethod = typeof (Enumerable).GetMethod("ToList").MakeGenericMethod(valueY.GetType().GetGenericArguments()[0]);
                var list2 = toListMethod.Invoke(null, new[] {valueY});
