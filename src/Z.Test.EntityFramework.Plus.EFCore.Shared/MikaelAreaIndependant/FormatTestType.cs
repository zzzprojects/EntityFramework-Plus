...
namespace Z.Test.EntityFramework.Plus.Mik_Area
{
    ...
    public class FormatTestType
    {
		...
		public void Test_1()
		{
			...
			var old2 = AuditManager.DefaultConfiguration.AutoSavePreAction;
			...
			try
			{
				...
				using (var context = new ModelAndContext.EntityContext())
				{ 
					var entityEFplus = context.AuditEntries.Include(x => x.Properties).First();

					Assert.AreEqual((125 + 500).ToString(), entityEFplus.Properties.Where(x => x.PropertyName =="ColumnInt").First().NewValueFormatted);
					Assert.AreEqual((1250 + 500).ToString(), entityEFplus.Properties.Where(x => x.PropertyName =="ColumnIntNullable").First().NewValueFormatted);
				} 
			...
			finally
			{
				...
				AuditManager.DefaultConfiguration.AutoSavePreAction = old2;
			}
		...
		public void Test_2()
		{
			...
			try
			{
				...
				using (var context = new ModelAndContext.EntityContext())
				{
					...
					Assert.AreEqual((125 + 500).ToString(),   entityEFplus.Properties.Where(x => x.PropertyName =="ColumnInt").First().NewValueFormatted);
					Assert.AreEqual(null, entityEFplus.Properties.Where(x => x.PropertyName =="ColumnIntNullable").First().NewValueFormatted);
				}
