...
namespace Z.Test.EntityFramework.Plus
{
    public partial class Audit_GetAudits_ByKey
    {
        ...
        public void Inheritance_TPH()
        {
            ...
            Inheritance_TPH_Cat auditedItem = null;

            using (var ctx = new TestContext())
            {
                ctx.Inheritance_TPH_Animals.Add(new Inheritance_TPH_Cat() { ColumnInt = 1, ColumnCat = 4 });
                ctx.Inheritance_TPH_Animals.Add(new Inheritance_TPH_Cat() { ColumnInt = 2, ColumnCat = 5 });
                auditedItem = (Inheritance_TPH_Cat)ctx.Inheritance_TPH_Animals.Add(new Inheritance_TPH_Cat() { ColumnInt = 3, ColumnCat = 6 });
            ...
            // UnitTest - Audit
            {
                using (var ctx = new TestContext())
                {
                    var auditEntries = ctx.AuditEntries.Where<Inheritance_TPH_Cat>(auditedItem.ID).ToList();
